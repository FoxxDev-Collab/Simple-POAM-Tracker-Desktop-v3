...
const MilestoneProgressBars: React.FC<MilestoneProgressProps> = ({ poams }) => {
  ...
  const itemsPerPage = 15; // Number of POAMs to show per page
  ...
  useEffect(() => {
    ...
    const pages = Math.ceil(poamsWithMilestones.length / itemsPerPage);
    setTotalPages(pages);
    ...
    // Get current page of POAMs
    const startIndex = currentPage * itemsPerPage;
    const endIndex = Math.min(startIndex + itemsPerPage, poamsWithMilestones.length);
    const currentPagePoams = poamsWithMilestones.slice(startIndex, endIndex);
    
    // Calculate milestone completion for each POAM
    const labels = currentPagePoams.map(poam => {
    ...
    const totalMilestones = currentPagePoams.map(poam => poam.milestones.length);
    const completedMilestones = currentPagePoams.map(poam => 
    ...
    setChartData({
      ...
      datasets: [
        ...
        },
        {
          ...
          data: currentPagePoams.map((_, index) => 
        ...
        tooltip: {
          ...
          callbacks: {
            afterTitle: (tooltipItems) => {
              ...
              return `ID: ${currentPagePoams[dataIndex].id}`;
            },
            footer: (tooltipItems) => {
              const dataIndex = tooltipItems[0].dataIndex;
              const currentPoam = currentPagePoams[dataIndex];
              const total = currentPoam.milestones.length;
              const completed = currentPoam.milestones.filter(m => m.status === 'Completed').length;
