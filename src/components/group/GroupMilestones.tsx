...
export default function GroupMilestones({ groupId }: GroupMilestonesProps) {
  ...
  useEffect(() => {
    console.log('GroupMilestones: Initializing component...');
    loadMilestones();
    loadSettings();
  }, [groupId]);
  ...
  const checkForCalendarNavigation = () => {
    ...
    if (highlightMilestoneId) {
      ...
      if (milestoneExists) {
        ...
        setTimeout(() => {
          console.log('GroupMilestones: Auto-clearing highlight after 10 seconds');
          setHighlightedMilestone(null);
        }, 10000);
      } else {
        console.warn('GroupMilestones: Milestone not found in current list:', highlightMilestoneId);
        // Clear session storage even if milestone not found
        sessionStorage.removeItem('highlightMilestone');
        sessionStorage.removeItem('highlightPOAM');
      }
    } else {
      console.log('GroupMilestones: No milestone to highlight');
    }
  };
  ...
  const clearHighlighting = () => {
    if (highlightedMilestone) {
      console.log('GroupMilestones: Clearing highlight due to user interaction');
      setHighlightedMilestone(null);
    }
  };
  ...
  const loadSettings = () => {
    try {
      console.log('GroupMilestones: Loading settings from localStorage...');
      ...
      console.error('GroupMilestones: Error loading settings:', error);
    }
  };

  // Apply filters and search whenever dependencies change
  useEffect(() => {
    console.log('GroupMilestones: Applying filters and search...');
  ...
  const loadMilestones = async () => {
    if (!groupId) {
      console.log('No group ID provided, skipping milestone load');
      setLoading(false);
      return;
    }

    try {
      console.log('GroupMilestones: Loading milestones from backend for group:', groupId);
      setLoading(true);
      const groupPoams = await invoke<GroupPOAM[]>('get_group_poams', { groupId });
      
      console.log('GroupMilestones: Received Group POAMs from backend:', groupPoams);
      
      // Extract all milestones from all Group POAMs
      const allMilestones: Milestone[] = [];
      
      if (groupPoams && groupPoams.length > 0) {
        groupPoams.forEach(poam => {
      ...
      console.log('GroupMilestones: Extracted milestones:', allMilestones);
      
      setMilestones(allMilestones);
    } catch (error) {
      console.error('GroupMilestones: Error loading milestones:', error);
      showToast('error', `Failed to load group milestones: ${error}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openMilestoneModal = (milestone: Milestone) => {
    console.log('GroupMilestones: Opening milestone modal with data:', milestone);
    ...
    if (!milestone.id) {
      console.error('GroupMilestones: Milestone is missing ID property:', milestone);
      showToast('error', 'Cannot edit milestone: Invalid data');
      return;
    }
    
    if (!milestone.poamId) {
      console.error('GroupMilestones: Milestone is missing POAM ID:', milestone);
  ...
  const handleMilestoneSave = async (updatedMilestone: Milestone) => {
    try {
      console.log('GroupMilestones: Saving updated milestone:', updatedMilestone);
      
      // Find the Group POAM this milestone belongs to
      if (updatedMilestone.poamId) {
        // Get the current Group POAM data
        const poamData = await invoke<GroupPOAM | null>('get_group_poam_by_id', { 
        ...
        if (poamData) {
          ...
          if (formattedDueDate && !formattedDueDate.includes('T')) {
            ...
            if (formattedDueDate.match(/^\d{4}-\d{2}-\d{2}$/)) {
              ...
              console.log(`GroupMilestones: Date kept as ${formattedDueDate} (no conversion needed)`);
            } else {
              ...
              try {
                ...
                if (parts.length === 3) {
                  ...
                  if (year && month && day) {
                    ...
                    console.log(`GroupMilestones: Date converted to ${formattedDueDate}`);
                  }
                }
              } catch (dateError) {
                console.error('GroupMilestones: Error formatting date:', dateError);
                // Keep the original value if conversion fails
              }
            }
          }
          
          // Update the milestone in the Group POAM's milestones array
          const updatedPoam = {
            ...poamData,
          ...
          console.log('GroupMilestones: Saving updated Group POAM with milestones:', updatedPoam);
          
          // Save the updated Group POAM with the modified milestone
          await invoke('update_group_poam', { 
            poam: updatedPoam
          });
          ...
          notifySystemEvent({
            type: 'sync',
            message: `Group milestone "${updatedMilestone.title}" updated successfully`,
            success: true
          });
          ...
          console.log('GroupMilestones: Milestone updated successfully');
          showToast('success', 'Group milestone updated successfully');
        } else {
          console.error('GroupMilestones: Group POAM not found for milestone update');
          showToast('error', 'Failed to update milestone: Group POAM not found');
          
          notifySystemEvent({
            type: 'error',
            message: `Failed to update group milestone "${updatedMilestone.title}": Group POAM not found`,
            success: false
          });
        }
      } else {
        console.error('GroupMilestones: No associated Group POAM for milestone update');
        showToast('error', 'Failed to update milestone: No associated Group POAM');
        
        notifySystemEvent({
          type: 'error',
          message: `Failed to update group milestone "${updatedMilestone.title}": No associated Group POAM`,
          success: false
        });
      }
    } catch (error) {
      console.error('GroupMilestones: Error updating milestone:', error);
      showToast('error', `Failed to update group milestone: ${error}`);
      
      notifySystemEvent({
        type: 'error',
        message: `Failed to update group milestone "${updatedMilestone.title}"`,
        success: false,
  ...
  const handleSort = (key: keyof Milestone) => {
    console.log('GroupMilestones: Sorting by:', key);
  ...
  const getUniqueValues = useCallback((key: keyof Milestone) => {
    console.log('GroupMilestones: Getting unique values for:', key);
    ...
    <div className="container-responsive space-y-6">
      ...
      <div className="responsive-header">
        <div className="flex items-center gap-3 title-row">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Group Milestones</h1>
            <p className="text-muted-foreground">
              Track progress across all Group POAMs
            </p>
          </div>
        </div>
      ...
      <div className="container-responsive bg-card rounded-lg p-6 space-y-4">
        ...
        <div className="w-full">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search group milestones..."
              value={searchQuery}
        ...
        <div className="flex flex-col md:flex-row gap-4 md:items-center">
          <div className="flex flex-col sm:flex-row gap-3 flex-1">
            ...
            <select
              ...
              className="px-3 py-2 border border-input rounded-md bg-background text-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/20 transition-colors w-full sm:w-auto"
            >
              <option value="">All Group POAMs</option>
      ...
      <div className="flex items-center justify-between text-sm">
        <div className="text-muted-foreground">
          Showing {filteredMilestones.length} of {milestones.length} group milestones
        </div>
      ...
      {loading ? (
        <div className="text-center py-8">
          ...
          <p className="mt-4 text-muted-foreground">Loading group milestones...</p>
        </div>
      ) : filteredMilestones.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-muted-foreground">
            {milestones.length === 0 ? 'No group milestones found. Create a Group POAM with milestones to get started.' : 'No milestones match the current filters.'}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full table-fixed">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  >
                    Group POAM {renderSortIndicator('poamTitle')}
                  </th>
                  <th 
