...
import {
  Save, Download, Eye, Edit3, Trash2, Search, Calendar, FileText, Users, Shield, AlertTriangle, CheckCircle, Clock, Plus
} from 'lucide-react';
import { Badge } from '../ui/badge';
import { Button } from '../ui/button';
import { getAllStpPrepLists, getStpPrepListById, deleteStpPrepList, updateStpPrepList } from '../../utils/tauriApi';
...
interface StpPrepList {
  ...
  asset_info: any;
  prep_status: string;
  selected_controls: any[];
  control_count: number;
}

interface PrepListDetailDialog {
  isOpen: boolean;
  prepList: StpPrepList | null;
  mode: 'view' | 'edit';
}

export default function STIGPrepListManager() {
  const { currentSystem } = useSystem();
  const { addToast } = useToast();

  const [loading, setLoading] = useState(false);
  const [prepLists, setPrepLists] = useState<StpPrepList[]>([]);
  ...
  // Load prep lists
  const loadPrepLists = useCallback(async () => {
    if (!currentSystem?.id) return;
    
    setLoading(true);
    try {
      const lists = await getAllStpPrepLists(currentSystem.id);
      setPrepLists(lists);
    } catch (error) {
      console.error('Error loading prep lists:', error);
      addToast('Failed to load prep lists', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleViewPrepList = useCallback(async (id: string) => {
    ...
    setLoading(true);
    try {
      const prepList = await getStpPrepListById(id, currentSystem.id);
      if (prepList) {
        setDetailDialog({
          isOpen: true,
          prepList,
  ...
  const handleDeletePrepList = useCallback(async (id: string, name: string) => {
    ...
    setLoading(true);
    try {
      await deleteStpPrepList(id, currentSystem.id);
      await loadPrepLists();
  ...
  }, [currentSystem?.id, addToast, loadPrepLists]);

  // Handle updating prep list
  const handleUpdatePrepList = useCallback(async (updatedPrepList: StpPrepList) => {
    if (!currentSystem?.id) return;
  ...
  const handleExportPrepList = useCallback(async (prepList: StpPrepList) => {
    setLoading(true);
    try {
      ...
      const filePath = await save({
        ...
        defaultPath: `stig-prep-${prepList.name.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.json`
      });
  ...
  const filteredPrepLists = prepLists.filter(prepList => {
    ...
    if (filter.trim()) {
      ...
      const matchesText = (
        ...
        prepList.stig_info?.title?.toLowerCase().includes(searchTerm)
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">STIG Prep Lists</h2>
          <p className="text-muted-foreground">
            Manage and export STIG preparation lists for {currentSystem?.name}
          </p>
        </div>
      </div>
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="bg-muted text-foreground font-semibold p-3 text-center border-b border-border">Vulnerabilities</th>
                      ...
                      <td className="p-3 border-b border-border">
                        <div className="text-sm">
                          <div className="font-medium text-foreground truncate max-w-xs">
                            {prepList.stig_info?.title || 'Unknown STIG'}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {prepList.asset_info?.asset_type || 'Unknown Asset'}
                          </div>
                        </div>
                      </td>
                      <td className="p-3 border-b border-border text-center">
                        <div className="font-medium text-primary">
                          {prepList.control_count || 0}
                        </div>
                      </td>
                      <td className="p-3 border-b border-border text-center">
                        {renderStatusBadge(prepList.prep_status)}
                      </td>
                      <td className="p-3 border-b border-border text-center">
                        <div className="text-sm text-muted-foreground">
              ...
              <div className="text-muted-foreground mb-4">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  {filter || statusFilter !== 'all' 
                    ...
                    : 'Create your first STIG prep list from the STIG Viewer tab.'
                  }
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-foreground">
                    {detailDialog.mode === 'edit' ? 'Edit' : 'View'} Prep List
                  </h3>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    {detailDialog.mode === 'edit' ? (
                      <select
                        value={detailDialog.prepList.prep_status}
                        onChange={(e) => setDetailDialog(prev => ({
                          ...prev,
                      ...
                      renderStatusBadge(detailDialog.prepList.prep_status)
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-muted p-4 rounded-lg space-y-2">
                    <div><strong>Title:</strong> {detailDialog.prepList.stig_info?.title || 'Unknown'}</div>
                    <div><strong>Version:</strong> {detailDialog.prepList.stig_info?.version || 'Unknown'}</div>
                    <div><strong>Classification:</strong> {detailDialog.prepList.stig_info?.classification || 'Unknown'}</div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-muted p-4 rounded-lg space-y-2">
                    <div><strong>Asset Type:</strong> {detailDialog.prepList.asset_info?.asset_type || 'Unknown'}</div>
                    {detailDialog.prepList.asset_info?.host_name && (
                      <div><strong>Host Name:</strong> {detailDialog.prepList.asset_info.host_name}</div>
                    )}
                    {detailDialog.prepList.asset_info?.host_ip && (
                      <div><strong>Host IP:</strong> {detailDialog.prepList.asset_info.host_ip}</div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <h4 className="text-lg font-semibold text-foreground mb-3">
                    Selected Vulnerabilities ({detailDialog.prepList.control_count})
                  </h4>
                  <div className="bg-muted rounded-lg max-h-60 overflow-y-auto">
                    {detailDialog.prepList.selected_controls?.map((control, index) => (
                      <div key={index} className="p-3 border-b border-border last:border-b-0">
                        <div className="font-medium text-foreground">{control.nist_control}</div>
                        {control.stigs?.[0] && (
                          <div className="text-sm text-muted-foreground mt-1">
                            {control.stigs[0].rule_title}
                          </div>
                        )}
                      </div>
                    ))}
